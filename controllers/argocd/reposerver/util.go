...
func GetRepoServerAddress(name string, namespace string) string {
	return argoutil.FqdnServiceRef(argoutil.NameWithSuffix(name, common.RepoServerControllerComponent), namespace, common.ArgoCDDefaultRepoServerPort)
}

func (rsr *RepoServerReconciler) TriggerRepoServerDeploymentRollout() error {
	name := argoutil.NameWithSuffix(rsr.Instance.Name, common.RepoServerControllerComponent)
