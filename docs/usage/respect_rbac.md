...
> Note: When respectRBAC is enabled on a cluster scoped Argo CD instance, the application-controller and server service accounts still require cluster-wide permissions on the following argoproj.io CRDs: Application, AppProject, and ApplicationSet. These permissions are necessary because the controllers manage watches for these resources independently of the watches Argo CD maintains for deployed resources.
