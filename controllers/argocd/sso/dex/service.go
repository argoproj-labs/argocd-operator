...
func (dr *DexReconciler) reconcileService() error {
	...
	return dr.reconService(req, argocdcommon.UpdateFnSvc(updateFn), ignoreDrift)

}

func (dr *DexReconciler) reconService(req networking.ServiceRequest, updateFn interface{}, ignoreDrift bool) error {
	desired, err := networking.RequestService(req)
	if err != nil {
		dr.Logger.Debug("reconcileService: one or more mutations could not be applied")
	...
	if err = controllerutil.SetControllerReference(dr.Instance, desired, dr.Scheme); err != nil {
		dr.Logger.Error(err, "reconcileService: failed to set owner reference for Service", "name", desired.Name, "namespace", desired.Namespace)
	}

	existing, err := networking.GetService(desired.Name, desired.Namespace, dr.Client)
	if err != nil {
		if !apierrors.IsNotFound(err) {
			return errors.Wrapf(err, "reconcileService: failed to retrieve Service %s in namespace %s", desired.Name, desired.Namespace)
		}

		if err = networking.CreateService(desired, dr.Client); err != nil {
			return errors.Wrapf(err, "reconcileService: failed to create Service %s in namespace %s", desired.Name, desired.Namespace)
		}
		dr.Logger.Info("service created", "name", desired.Name, "namespace", desired.Namespace)
		return nil
	}
	...
	if err = networking.UpdateService(existing, dr.Client); err != nil {
		return errors.Wrapf(err, "reconcileService: failed to update service %s", existing.Name)
	}

	dr.Logger.Info("service updated", "name", existing.Name, "namespace", existing.Namespace)
	return nil
}

// deleteService will delete service with given name.
func (dr *DexReconciler) deleteService(name, namespace string) error {
	if err := networking.DeleteService(name, namespace, dr.Client); err != nil {
		if apierrors.IsNotFound(err) {
			return nil
		}
		return errors.Wrapf(err, "deleteService: failed to delete service %s in namespace %s", name, namespace)
	}
	dr.Logger.Info("service deleted", "name", name, "namespace", namespace)
	return nil
}
