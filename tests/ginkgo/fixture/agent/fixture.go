...
func VerifyExpectedResourcesExist(params VerifyExpectedResourcesExistParams) {
	...
	if len(params.SecretNames) > 4 && params.SecretNames[4] != "" {
		Eventually(&corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Name:      params.SecretNames[4],
	...
	defer func() { _ = k8sClient.Delete(ctx, params.ClusterRole) }()

	Eventually(params.ClusterRoleBinding).Should(k8sFixture.ExistByName())
	defer func() { _ = k8sClient.Delete(ctx, params.ClusterRoleBinding) }()
