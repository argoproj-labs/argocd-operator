...
func (r *ReconcileArgoCD) reconcileDexServiceAccount(cr *argoprojv1a1.ArgoCD) error {
	if isDexDisabled() || cr.Spec.SSO == nil || reflect.DeepEqual(cr.Spec.SSO.Dex, argoprojv1a1.ArgoCDDexSpec{}) || !cr.Spec.SSO.Dex.OpenShiftOAuth {
...
func (r *ReconcileArgoCD) reconcileServiceAccount(name string, cr *argoprojv1a1.ArgoCD) (*corev1.ServiceAccount, error) {
	...
	if exists {
		if name == dexServer && (isDexDisabled() || cr.Spec.SSO == nil || cr.Spec.SSO.Provider != argoprojv1a1.SSOProviderTypeDex) {
			log.Info("deleting the existing Dex service account because dex is either disabled or not configured")
