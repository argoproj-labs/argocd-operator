...
func newRole(name string, rules []v1.PolicyRule, cr *argoproj.ClusterArgoCD) *v1.Role {
	return &v1.Role{
...
func newRoleForApplicationSourceNamespaces(namespace string, rules []v1.PolicyRule, cr *argoproj.ClusterArgoCD) *v1.Role {
	return &v1.Role{
		ObjectMeta: metav1.ObjectMeta{
...
func generateResourceName(argoComponentName string, cr *argoproj.ClusterArgoCD) string {
...
func (r *ReconcileClusterArgoCD) reconcileRoles(cr *argoproj.ClusterArgoCD) error {
...
func (r *ReconcileClusterArgoCD) reconcileRole(name string, policyRules []v1.PolicyRule, cr *argoproj.ClusterArgoCD) ([]*v1.Role, error) {
	var roles []*v1.Role
	...
	for _, namespace := range r.ManagedNamespaces.Items {
		...
		list := &argoproj.ClusterArgoCDList{}
...
func (r *ReconcileClusterArgoCD) reconcileRoleForApplicationSourceNamespaces(name string, policyRules []v1.PolicyRule, cr *argoproj.ClusterArgoCD) error {
...
func (r *ReconcileClusterArgoCD) reconcileClusterRole(componentName string, policyRules []v1.PolicyRule, cr *argoproj.ClusterArgoCD) (*v1.ClusterRole, error) {

	// if policyRules is empty and aggregatedClusterRoles is not enabled, then return nil
	if len(policyRules) == 0 && !cr.Spec.AggregatedClusterRoles {
		...
		if err := r.Get(context.TODO(), types.NamespacedName{Name: expectedClusterRole.Name}, existingClusterRole); err == nil {
			argoutil.LogResourceDeletion(log, existingClusterRole, "policy rules are empty")
	...
	useCustomClusterRole, err := checkCustomClusterRoleMode(r, cr, componentName, true)
...
func checkCustomClusterRoleMode(r *ReconcileClusterArgoCD, cr *argoproj.ClusterArgoCD, componentName string, allowed bool) (bool, error) {
...
func configureAggregatedClusterRole(cr *argoproj.ClusterArgoCD, clusterRole *v1.ClusterRole, componentName string) {
