...
var _ = Describe("GitOps Operator Parallel E2E Tests", func() {
	...
	Context("1-126_source_hydrator", func() {
		...
		var (
			...
			helmDrySourceFiles = map[string]string{
				"helm/Chart.yaml": `
apiVersion: v2
name: source-hydrator-helm
version: 0.1.0
`,
				"helm/values.yaml": `
foo: base
...
kind: ConfigMap
metadata:
  name: source-hydrator-helm-test
data:
  foo: {{ .Values.foo | quote }}
`,
			}
			kustomizeDrySourceFiles = map[string]string{
...
`,
		...
		It("hydrate helm to another directory via https", func() {
				...
				Spec: argov1beta1api.ArgoCDSpec{
					TLS: argov1beta1api.ArgoCDTLSSpec{
						InitialCerts: map[string]string{
							server.TLSHostKey(): string(server.GetCACert()),
						},
					},
					SourceHydrator: argov1beta1api.ArgoCDSourceHydratorSpec{
						Enabled: ptr.To(true),
			...
			Eventually(app, "2m", "5s").Should(appFixture.HaveOperationStatePhase(synccommon.OperationSucceeded))
			Expect(app).Should(appFixture.HaveSyncStatusCode(argocdv1alpha1.SyncStatusCodeSynced))
			Expect(app).Should(appFixture.HaveHealthStatusCode(health.HealthStatusHealthy))

			By("verifying helm parameter was applied to the synced ConfigMap")
			syncedCM := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "source-hydrator-helm-test",
					Namespace: ns.Name,
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(manifest).To(ContainSubstring("kind: ConfigMap"))
				g.Expect(manifest).To(ContainSubstring("name: source-hydrator-helm-test"))
				g.Expect(manifest).To(ContainSubstring("foo: helm"))
				g.Expect(manifest).NotTo(ContainSubstring("foo: base"))
			}).Should(Succeed())
		})
	})
