...
func ReconcilePrincipalRole(client client.Client, compName string, cr *argoproj.ArgoCD, scheme *runtime.Scheme) (*v1.Role, error) {
	...
	expectedPolicyRule := buildPolicyRuleForPrincipalRole()
...
func ReconcilePrincipalClusterRoles(client client.Client, compName string, cr *argoproj.ArgoCD, scheme *runtime.Scheme) (*v1.ClusterRole, error) {
	...
	expectedPolicyRule := buildPolicyRuleForPrincipalClusterRole()
	...
	//if err := controllerutil.SetControllerReference(cr, clusterRole, scheme); err != nil {
	//	return nil, fmt.Errorf("failed to set ArgoCD CR %s as owner for clusterRole %s: %v", cr.Name, clusterRole.Name, err)
	//}
...
func buildPolicyRuleForPrincipalRole() []v1.PolicyRule {
...
func buildPolicyRuleForPrincipalClusterRole() []v1.PolicyRule {
	return []v1.PolicyRule{
		{
			APIGroups: []string{
				"argoproj.io",
