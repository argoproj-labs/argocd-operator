...
func TestReconcileArgoCD_reconcileServiceAccountPermissions(t *testing.T) {
	...
	assert.NoError(t, r.Get(context.TODO(), types.NamespacedName{Name: expectedName, Namespace: a.Spec.ControlPlaneNamespace}, reconcileRoleBinding))
	assert.NoError(t, r.Get(context.TODO(), types.NamespacedName{Name: expectedName, Namespace: a.Spec.ControlPlaneNamespace}, reconciledServiceAccount))
	...
	// fetch it
	dirtyRole := &v1.Role{}
	assert.NoError(t, r.Get(context.TODO(), types.NamespacedName{Name: expectedName, Namespace: a.Spec.ControlPlaneNamespace}, dirtyRole))
...
func TestReconcileArgoCD_reconcileServiceAccountClusterPermissions(t *testing.T) {
	logf.SetLogger(ZapLogger(true))
	a := makeTestClusterArgoCDInNamespace("argocd-sa-cluster-permissions-test")
	...
	workloadIdentifier := "xrb"
	expectedClusterRoleBindingName := fmt.Sprintf("%s-%s-%s", a.Name, a.Spec.ControlPlaneNamespace, workloadIdentifier)
	expectedClusterRoleName := fmt.Sprintf("%s-%s-%s", a.Name, a.Spec.ControlPlaneNamespace, workloadIdentifier)
	...
	assert.NoError(t, r.reconcileServiceAccountClusterPermissions(workloadIdentifier, []v1.PolicyRule{}, a))

	//Service account should be created but no ClusterRole/ClusterRoleBinding should be created
	assert.NoError(t, r.Get(context.TODO(), types.NamespacedName{Name: expectedNameSA, Namespace: a.Spec.ControlPlaneNamespace}, reconcileServiceAccount))
