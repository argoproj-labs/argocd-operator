...
func (r *ReconcileClusterArgoCD) reconcileNamespaceManagement(argocd *argoproj.ClusterArgoCD) error {
	...
	for _, nm := range nmList.Items {
		...
		if nm.Spec.ManagedBy != argocd.Spec.ControlPlaneNamespace {
		...
		if matchesNamespaceManagementRules(allowedNsPatterns, namespace) {
			...
			message = fmt.Sprintf("Namespace %s is not permitted for management by ArgoCD instance %s based on NamespaceManagement rules", namespace, argocd.Spec.ControlPlaneNamespace)
	...
	managedNamespaces = append(managedNamespaces, corev1.Namespace{
		ObjectMeta: metav1.ObjectMeta{Name: argocd.Spec.ControlPlaneNamespace},
	})
...
func (r *ReconcileClusterArgoCD) disableNamespaceManagement(argocd *argoproj.ClusterArgoCD, k8sClient kubernetes.Interface) error {
	ctx := context.TODO()
	...
	for _, ns := range argocd.Spec.NamespaceManagement {
		...
		for _, nsName := range matchedNamespaces {
			...
			if allowManaged {
				...
				if err := deleteManagedNamespaceFromClusterSecret(argocd.Spec.ControlPlaneNamespace, nsName, k8sClient); err != nil {
