...
var imageTests = []struct {
	...
	opts      []argoCDOpt
	want      string
	imageFunc func(a *argoproj.ClusterArgoCD) string
}{
	{
	...
	},
	{
		...
		imageFunc: getDexContainerImage,
		want:      dexTestImage,
		opts: []argoCDOpt{func(a *argoproj.ClusterArgoCD) {
	...
	},
	{
		...
		imageFunc: getArgoContainerImage,
		want:      argoTestImage, opts: []argoCDOpt{func(a *argoproj.ClusterArgoCD) {
	...
	},
	{
		...
		imageFunc: getRepoServerContainerImage,
		want:      argoTestImageOtherVersion, opts: []argoCDOpt{func(a *argoproj.ClusterArgoCD) {
			a.Spec.Image = "testing/argocd"
			a.Spec.Version = "test"
		}},
	},
...
func TestGetArgoServerURI(t *testing.T) {
	for _, tt := range argoServerURITests {
		t.Run(tt.name, func(t *testing.T) {
			cr := makeTestClusterArgoCD(tt.opts...)
...
func TestRemoveDeletionFinalizer(t *testing.T) {
	t.Run("ArgoCD resource present", func(t *testing.T) {
		a := makeTestClusterArgoCD(addFinalizer(common.ArgoCDDeletionFinalizer))
...
func TestRemoveManagedNamespaceFromClusterSecretAfterDeletion(t *testing.T) {
	...
	_, err := testClient.CoreV1().Secrets(a.Spec.ControlPlaneNamespace).Create(context.TODO(), secret, metav1.CreateOptions{})
	assert.NoError(t, err)

	// run deleteManagedNamespaceFromClusterSecret
	assert.NoError(t, deleteManagedNamespaceFromClusterSecret(a.Spec.ControlPlaneNamespace, testNameSpace, testClient))

	// secret should still exists with updated list of namespaces
	s, err := testClient.CoreV1().Secrets(a.Spec.ControlPlaneNamespace).Get(context.TODO(), secret.Name, metav1.GetOptions{})
	assert.NoError(t, err)
...
func TestRemoveManagedByLabelFromNamespaces(t *testing.T) {
	...
	err = r.removeManagedByLabelFromNamespaces(a.Spec.ControlPlaneNamespace)
	assert.NoError(t, err)
...
// TestReconcileClusterArgoCD_reconcileDexOAuthClientSecret This test make sures that if dex is enabled a service account is created with token stored in a secret which is used for oauth
func TestReconcileClusterArgoCD_reconcileDexOAuthClientSecret(t *testing.T) {
	...
	assert.NoError(t, argoutil.FetchObject(r.Client, a.Spec.ControlPlaneNamespace, sa.Name, sa))
	tokenExists := false
...
func TestNamespaceManagementHandlers(t *testing.T) {
	...
	setupRBACAndSecret := func(t *testing.T, client *testclient.Clientset, argo *argoproj.ClusterArgoCD, namespace string) string {
		t.Helper()
	...
	t.Run("HandleArgoCDNamespaceManagementUpdate test", func(t *testing.T) {
		argoCDOld := makeTestClusterArgoCD(func(cr *argoproj.ClusterArgoCD) {
		...
		argoCDNew := makeTestClusterArgoCD(func(cr *argoproj.ClusterArgoCD) {
			cr.Name = "argocd-2"
			cr.Spec.NamespaceManagement = []argoproj.ManagedNamespaces{
				{Name: "another-ns", AllowManagedBy: true},
			}
	...
	t.Run("HandleNamespaceManagementUpdate test", func(t *testing.T) {
		...
		_, err = client.CoreV1().Secrets(argoCD.Spec.ControlPlaneNamespace).Get(context.TODO(), secretName, metav1.GetOptions{})
