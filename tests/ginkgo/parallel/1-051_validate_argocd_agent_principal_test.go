...
var _ = Describe("GitOps Operator Parallel E2E Tests", func() {
	...
	Context("1-051_validate_argocd_agent_principal", func() {
		...
		It("verifies that enabling Argo CD agent principal via ArgoCD CR causes the correct resources to be created", func() {
			...
			for _, serviceName := range servicesExist {
				By("verifying Service '" + serviceName + "' exists and is a LoadBalancer")
				...
				if serviceName == "argocd-agent-principal" || serviceName == "argocd-repo-server" {
