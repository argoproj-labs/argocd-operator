...
func checkExpectedDeploymentValues(t *testing.T, r *ReconcileClusterArgoCD, deployment *appsv1.Deployment, sa *v1.ServiceAccount, extraVolumes *[]v1.Volume, extraVolumeMounts *[]v1.VolumeMount, a *argoproj.ClusterArgoCD) {
	...
	if a.Spec.ApplicationSet.SCMRootCAConfigMap != "" {
		...
		exists, err := argoutil.IsObjectFound(r.Client, a.Spec.ControlPlaneNamespace, common.ArgoCDAppSetGitlabSCMTLSCertsConfigMapName, a)
		assert.Nil(t, err)
		if exists {
...
func TestReconcileApplicationSet_Deployments_resourceRequirements(t *testing.T) {
	logf.SetLogger(ZapLogger(true))
	a := makeTestClusterArgoCDWithResources()
...
func TestReconcileApplicationSet_Deployments_SpecOverride(t *testing.T) {
	...
	tests := []struct {
		...
		argocdField            argoproj.ClusterArgoCDSpec
...
func TestReconcileApplicationSet_Deployments_Command(t *testing.T) {
	...
	tests := []struct {
		name           string
		argocdSpec     argoproj.ClusterArgoCDSpec
		expectedCmd    []string
...
func TestReconcileApplicationSet_ClusterRBACCreationAndCleanup(t *testing.T) {
	...
	// test: ClusterArgoCD is cluster-scoped, resources should be created
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, ns := range test.existInNs {
				...
				assert.True(t, found)
				assert.Equal(t, a.Spec.ControlPlaneNamespace, val)
			}
...
func TestReconcileApplicationSet_ServiceWithLongName(t *testing.T) {
	...
	err = r.List(context.TODO(), serviceList, client.InNamespace(a.Spec.ControlPlaneNamespace))
	assert.NoError(t, err)
...
func TestArgoCDApplicationSet_getApplicationSetSourceNamespaces(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			a := makeTestClusterArgoCD()
			a.Spec.SourceNamespaces = test.sourceNamespaces
			resObjs := []client.Object{a}
			subresObjs := []client.Object{a}
			runtimeObjs := []runtime.Object{}
			sch := makeTestReconcilerScheme(argoproj.AddToScheme)
...
func TestGetApplicationSetContainerImage(t *testing.T) {
	...
	cr.Spec = argoproj.ClusterArgoCDSpec{}
