...
func TestReconcileClusterArgoCD_Reconcile_with_deleted(t *testing.T) {
	argoutil.SetRouteAPIFound(true) // Setup Route API for tests that call full reconciler
	logf.SetLogger(ZapLogger(true))
	a := makeTestClusterArgoCD(deletedAt(time.Now()))
...
// TestReconcileClusterArgoCD_DexWorkloads verifies that when dex is enabled, that the appropriate operator resources are created. When dex is disabled, the objects are verified to be removed.
func TestReconcileClusterArgoCD_DexWorkloads(t *testing.T) {
	argoutil.SetRouteAPIFound(true) // Setup Route API for tests that call full reconciler
	logf.SetLogger(ZapLogger(true))
	...
	dexRole := &rbacv1.Role{ObjectMeta: metav1.ObjectMeta{Name: "argocd-argocd-dex-server", Namespace: a.Spec.ControlPlaneNamespace}}
	objectsToVerify = append(objectsToVerify, dexRole)

	dexRoleBinding := &rbacv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "argocd-argocd-dex-server", Namespace: a.Spec.ControlPlaneNamespace}}
	objectsToVerify = append(objectsToVerify, dexRoleBinding)

	dexServiceAccount := &corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Name: "argocd-argocd-dex-server", Namespace: a.Spec.ControlPlaneNamespace}}
	objectsToVerify = append(objectsToVerify, dexServiceAccount)

	dexService := &corev1.Service{ObjectMeta: metav1.ObjectMeta{Name: "argocd-dex-server", Namespace: a.Spec.ControlPlaneNamespace}}
	objectsToVerify = append(objectsToVerify, dexService)

	dexDeployment := &corev1.Service{ObjectMeta: metav1.ObjectMeta{Name: "argocd-dex-server", Namespace: a.Spec.ControlPlaneNamespace}}
	...
	err = r.List(context.TODO(), &secretList, client.InNamespace(a.Spec.ControlPlaneNamespace))
	assert.NoError(t, err)
...
func TestReconcileClusterArgoCD_Reconcile(t *testing.T) {
	argoutil.SetRouteAPIFound(true) // Setup Route API for tests that call full reconciler
	logf.SetLogger(ZapLogger(true))
	a := makeTestClusterArgoCD()
...
func TestReconcileClusterArgoCD_LabelSelector(t *testing.T) {
	argoutil.SetRouteAPIFound(true) // Setup Route API for tests that call full reconciler
	logf.SetLogger(ZapLogger(true))
	//ctx := context.Background()
	a := makeTestClusterArgoCD(func(ac *argoproj.ClusterArgoCD) {
		ac.Name = "argo-test-1"
		ac.Labels = map[string]string{"foo": "bar"}
	})
	b := makeTestClusterArgoCD(func(ac *argoproj.ClusterArgoCD) {
		ac.Name = "argo-test-2"
		ac.Labels = map[string]string{"testfoo": "testbar"}
	})
	c := makeTestClusterArgoCD(func(ac *argoproj.ClusterArgoCD) {
		ac.Name = "argo-test-3"
	})
	...
	assert.NoError(t, createNamespace(rt, a.Spec.ControlPlaneNamespace, ""))
...
func TestReconcileClusterArgoCD_Reconcile_RemoveManagedByLabelOnArgocdDeletion(t *testing.T) {
	argoutil.SetRouteAPIFound(true) // Setup Route API for tests that call full reconciler
	logf.SetLogger(ZapLogger(true))
	...
	for _, test := range tests {
		t.Run(test.testName, func(t *testing.T) {
			a := makeTestClusterArgoCD(deletedAt(time.Now()), addFinalizer(common.ArgoCDDeletionFinalizer))
			...
			nsArgocd := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{
				Name: a.Spec.ControlPlaneNamespace,
			}}
			...
			ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{
				...
				Labels: map[string]string{
					common.ArgoCDManagedByLabel: a.Spec.ControlPlaneNamespace,
				}},
			}
			...
			if test.isRemoveManagedByLabelOnArgoCDDeletionSet {
				...
				assert.Equal(t, ns.Labels[common.ArgoCDManagedByLabel], a.Spec.ControlPlaneNamespace)
			}
		})
	}
}
...
func TestReconcileClusterArgoCD_CleanUp(t *testing.T) {
	argoutil.SetRouteAPIFound(true) // Setup Route API for tests that call full reconciler
	logf.SetLogger(ZapLogger(true))
	a := makeTestClusterArgoCD(deletedAt(time.Now()), addFinalizer(common.ArgoCDDeletionFinalizer))
	...
	ns := &corev1.Namespace{}
	assert.NoError(t, r.Get(context.TODO(), types.NamespacedName{Name: a.Spec.ControlPlaneNamespace}, ns))
	if _, ok := ns.Labels[common.ArgoCDManagedByLabel]; ok {
		t.Errorf("Expected the label[%v] to be removed from the namespace[%v]", common.ArgoCDManagedByLabel, a.Spec.ControlPlaneNamespace)
	}
}
...
func clusterResources(argocd *argoproj.ClusterArgoCD) []client.Object {
...
func TestReconcileClusterArgoCD_Status_Condition(t *testing.T) {
	argoutil.SetRouteAPIFound(true) // Setup Route API for tests that call full reconciler
	logf.SetLogger(ZapLogger(true))
	...
	assert.NoError(t, rt.Get(context.TODO(), types.NamespacedName{Name: a.Name, Namespace: a.Spec.ControlPlaneNamespace}, a))
...
func TestReconcileClusterArgoCD_Cleanup_RBACs_When_NamespaceManagement_Disabled(t *testing.T) {
	argoutil.SetRouteAPIFound(true) // Setup Route API for tests that call full reconciler
	namespace := testNamespace
	argoCD := makeTestClusterArgoCD()
	...
	nsMgmt := &argoproj.NamespaceManagement{
		...
		Spec: argoproj.NamespaceManagementSpec{
			ManagedBy: argoCD.Spec.ControlPlaneNamespace,
		},
	}
	...
	assert.NoError(t, createNamespace(r, argoCD.Spec.ControlPlaneNamespace, ""))
	...
	_, err = client.CoreV1().Secrets(argoCD.Spec.ControlPlaneNamespace).Create(context.TODO(), secret, metav1.CreateOptions{})
	assert.NoError(t, err)

	req := reconcile.Request{
		NamespacedName: types.NamespacedName{
			Name:      argoCD.Name,
			Namespace: argoCD.Spec.ControlPlaneNamespace,
		},
	}
	...
	updatedSecret, err := client.CoreV1().Secrets(argoCD.Spec.ControlPlaneNamespace).Get(context.TODO(), secret.Name, metav1.GetOptions{})
	assert.NoError(t, err)
