...
func TestReconcileRouteTLSConfig(t *testing.T) {
	...
	tt := []struct {
		...
		updateArgoCD    func(cr *argoproj.ClusterArgoCD)
		createResources func(k8sClient client.Client, cr *argoproj.ClusterArgoCD)
	}{
		{
	...
	for _, test := range tt {
		t.Run(test.name, func(t *testing.T) {
			argoCD := makeClusterArgoCD(test.updateArgoCD)
			...
			route := &routev1.Route{}
			err = reconciler.Get(ctx, types.NamespacedName{Name: argoCD.Name + "-server", Namespace: argoCD.Spec.ControlPlaneNamespace}, route)
			assert.Nil(t, err)
...
func makeClusterArgoCD(opts ...func(*argoproj.ClusterArgoCD)) *argoproj.ClusterArgoCD {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.expectErr {
				...
				assert.NoError(t, err)
				route := routev1.Route{}
				err = argoutil.FetchObject(r.Client, a.Spec.ControlPlaneNamespace, test.routeName, &route)
				assert.NoError(t, err)
