...
var (
	...
	// Cache metrics
	SecretsCached = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "argocd_operator_secrets_cached_total",
		Help: "Total number of secrets cached by the operator",
	})

	ConfigMapsCached = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "argocd_operator_configmaps_cached_total",
		Help: "Total number of configmaps cached by the operator",
	})
)

func init() {
	metrics.Registry.MustRegister(ActiveInstancesTotal, ActiveInstancesByPhase, ActiveInstanceReconciliationCount, ReconcileTime, SecretsCached, ConfigMapsCached)
}
