...
func getDexContainerImage(cr *argoproj.ClusterArgoCD) string {
...
func (r *ReconcileClusterArgoCD) getDexOAuthRedirectURI(cr *argoproj.ClusterArgoCD) (string, error) {
...
func getDexOAuthClientID(cr *argoproj.ClusterArgoCD) string {
	return fmt.Sprintf("system:serviceaccount:%s:%s", cr.Spec.ControlPlaneNamespace, fmt.Sprintf("%s-%s", cr.Name, common.ArgoCDDefaultDexServiceAccountName))
}

// getDexResources will return the ResourceRequirements for the Dex container.
func getDexResources(cr *argoproj.ClusterArgoCD) corev1.ResourceRequirements {
...
func getDexConfig(cr *argoproj.ClusterArgoCD) string {
