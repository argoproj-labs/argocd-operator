...
func TestReconcileNotifications_CreateClusterRole(t *testing.T) {
	logf.SetLogger(ZapLogger(true))
	a := makeTestClusterArgoCDInNamespace("noti-test", func(a *argoproj.ClusterArgoCD) {
		a.Spec.Notifications.Enabled = true
	})
	resObjs := []client.Object{a}
	subresObjs := []client.Object{a}
...
func TestReconcileNotifications_CreateClusterRoleBinding(t *testing.T) {
	...
	sa := &v1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Name: "sa-name", Namespace: a.Spec.ControlPlaneNamespace}}
...
func TestReconcileNotifications_CreateSecret(t *testing.T) {
	...
	assert.NoError(t, err)
	secret := &v1.Secret{}
	err = r.Get(context.TODO(), types.NamespacedName{Name: "argocd-notifications-secret", Namespace: a.Spec.ControlPlaneNamespace}, secret)
