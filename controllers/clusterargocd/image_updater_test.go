...
func TestReconcileImageUpdater_CreateSecret(t *testing.T) {
	...
	assert.NoError(t, err)
	secret := &v1.Secret{}
	err = r.Get(context.TODO(), types.NamespacedName{Name: "argocd-image-updater-secret", Namespace: a.Spec.ControlPlaneNamespace}, secret)
...
func TestReconcileImageUpdater_CreateConfigMap(t *testing.T) {
	...
	err = r.Get(context.TODO(), types.NamespacedName{Name: "argocd-image-updater-config", Namespace: a.Spec.ControlPlaneNamespace}, configMap)
	assertNotFound(t, err)
}
