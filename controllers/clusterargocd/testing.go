...
func makeTestReconciler(client client.Client, sch *runtime.Scheme, k8sClient kubernetes.Interface) *ReconcileClusterArgoCD {
	return &ReconcileClusterArgoCD{
		Client:    client,
		Scheme:    sch,
		K8sClient: k8sClient,
...
func makeTestClusterArgoCD(opts ...argoCDOpt) *argoproj.ClusterArgoCD {
	a := &argoproj.ClusterArgoCD{
		...
		Spec: argoproj.ClusterArgoCDSpec{
			ControlPlaneNamespace: testNamespace,
		},
	}
	for _, o := range opts {
		o(a)
...
func makeTestClusterArgoCDInNamespace(ns string, opts ...argoCDOpt) *argoproj.ClusterArgoCD {
	a := &argoproj.ClusterArgoCD{
		ObjectMeta: metav1.ObjectMeta{
			Name: testArgoCDName,
		},
...
func makeTestClusterArgoCDWithResources(opts ...argoCDOpt) *argoproj.ClusterArgoCD {
	a := &argoproj.ClusterArgoCD{
		ObjectMeta: metav1.ObjectMeta{
			Name: testArgoCDName,
		},
...
func createNamespace(r *ReconcileClusterArgoCD, n string, managedBy string) error {
...
func createNamespaceManagedByClusterArgoCDLabel(r *ReconcileClusterArgoCD, n string, managedBy string) error {
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: n}}
	if managedBy != "" {
