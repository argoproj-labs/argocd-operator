...
func TestRedisNetworkPolicy(t *testing.T) {
	...
	err = r.Get(context.TODO(), client.ObjectKey{Name: fmt.Sprintf("%s-%s", a.Name, RedisNetworkPolicy), Namespace: a.Spec.ControlPlaneNamespace}, np)
	assert.NoError(t, err)
...
func TestRedisHANetworkPolicy(t *testing.T) {
	...
	// Check if the network policy was created
	np := &networkingv1.NetworkPolicy{}
	err = r.Get(context.TODO(), client.ObjectKey{Name: fmt.Sprintf("%s-%s", a.Name, RedisHANetworkPolicy), Namespace: a.Spec.ControlPlaneNamespace}, np)
	assert.NoError(t, err)
