...
func (r *ReconcileClusterArgoCD) internalReconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, *argoproj.ClusterArgoCD, *argoproj.ArgoCDStatus, error) {
	...
	// Default ControlPlaneNamespace to Name if not set
	if argocd.Spec.ControlPlaneNamespace == "" {
		argocd.Spec.ControlPlaneNamespace = argocd.Name
		if err := r.Client.Update(ctx, argocd); err != nil {
			return reconcile.Result{}, argocd, argoCDStatus, fmt.Errorf("failed to default ControlPlaneNamespace: %w", err)
		}
		// Return immediately to let the next reconcile loop handle the updated object
		return reconcile.Result{Requeue: true}, argocd, argoCDStatus, nil
	}
	...
	// Match the value of labelSelector from ReconcileClusterArgoCD to labels from the argocd instance
	...
	if _, ok := ActiveInstanceMap[argocd.Spec.ControlPlaneNamespace]; !ok {
		if newPhase != "" {
			ActiveInstanceMap[argocd.Spec.ControlPlaneNamespace] = newPhase
		...
		if oldPhase := ActiveInstanceMap[argocd.Spec.ControlPlaneNamespace]; oldPhase != newPhase {
			ActiveInstanceMap[argocd.Spec.ControlPlaneNamespace] = newPhase
			ActiveInstancesByPhase.WithLabelValues(newPhase).Inc()
	...
	ActiveInstanceReconciliationCount.WithLabelValues(argocd.Spec.ControlPlaneNamespace).Inc()
	...
	if argocd.GetDeletionTimestamp() != nil {
		...
		delete(ActiveInstanceMap, argocd.Spec.ControlPlaneNamespace)
		ActiveInstancesByPhase.WithLabelValues(newPhase).Dec()
		ActiveInstancesTotal.Dec()
		ActiveInstanceReconciliationCount.DeleteLabelValues(argocd.Spec.ControlPlaneNamespace)
		ReconcileTime.DeletePartialMatch(prometheus.Labels{"namespace": argocd.Spec.ControlPlaneNamespace})

		// Remove any local user token renewal timers for the namespace
		r.cleanupNamespaceTokenTimers(argocd.Spec.ControlPlaneNamespace)
		...
		if argocd.IsDeletionFinalizerPresent() {
			...
			if isRemoveManagedByLabelOnArgoCDDeletion() {
				if err := r.removeManagedByLabelFromNamespaces(argocd.Spec.ControlPlaneNamespace); err != nil {
					return reconcile.Result{}, argocd, argoCDStatus, fmt.Errorf("failed to remove label from namespace[%v], error: %w", argocd.Spec.ControlPlaneNamespace, err)
				}
			}
			...
			delete(DeprecationEventEmissionTracker, argocd.Spec.ControlPlaneNamespace)
		}
	...
	if isNamespaceManagementEnabled() {
		...
		for _, nsMgmt := range nsMgmtList.Items {
			...
			if nsMgmt.Spec.ManagedBy != argocd.Spec.ControlPlaneNamespace {
				continue
			}
			...
			if err := deleteManagedNamespaceFromClusterSecret(argocd.Spec.ControlPlaneNamespace, nsMgmt.Namespace, k8sClient); err != nil {
