...
func (r *ArgoCDExportReconciler) reconcileExport(cr *argoprojv1a1.ArgoCDExport) error {
	exportLogger.Info("reconciling export secret")
	if err := r.reconcileExportSecret(cr); err != nil {
		return err
	}

	if cr.Spec.Schedule != nil && len(*cr.Spec.Schedule) > 0 {
		exportLogger.Info("reconciling export cronjob")
		if err := r.reconcileCronJob(cr); err != nil {
			return err
		}
	} else {
		exportLogger.Info("reconciling export job")
...
func (r *ArgoCDExportReconciler) validateExport(cr *argoprojv1a1.ArgoCDExport) error {
